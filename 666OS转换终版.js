  ...
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/generate' && request.method === 'POST') {
      try {
        ...
        const yaml = generateYamlConfig(subUrl, version);
    ...
    // 2. GET /sub 接口：供 Clash/Mihomo 客户端直接订阅更新
 ...
 * 核心逻辑：核心替换并动态填充订阅链接
 */
function generateYamlConfig(subUrl, version) {
  let yaml = version === 'pro' ? PRO_TEMPLATE : LITE_TEMPLATE;

  // 1. 自动追加 flag=meta 参数
  let finalSubUrl = subUrl;
  if (!finalSubUrl.includes('flag=')) {
    finalSubUrl += (finalSubUrl.includes('?') ? '&' : '?') + 'flag=meta';
  }

  // 2. 替换模板中的占位符
  yaml = yaml.replace(/__SUB_URL_PLACEHOLDER__/g, finalSubUrl);

  return yaml;
}

// ==================== Lite 模板 (包含自动选择分组) ====================
const LITE_TEMPLATE = `# 666OS Lite 自动选择增强版
...
# 这里加入了 [自动选择] 为第一优先级策略
x-oversea: &oversea [自动选择, 故障转移, 全球手动, 香港策略, 台湾策略, 日本策略, 狮城策略, 韩国策略, 美国策略, DIRECT]
x-direct: &direct [DIRECT, 自动选择, 故障转移, 全球手动, 香港策略, 台湾策略, 日本策略, 狮城策略, 韩国策略, 美国策略]
x-us-first: &us-first [美国策略, 自动选择, 故障转移, 全球手动, 香港策略, 台湾策略, 日本策略, 狮城策略, 韩国策略, DIRECT]
x-sg-first: &sg-first [狮城策略, 自动选择, 故障转移, 全球手动, 香港策略, 台湾策略, 日本策略, 韩国策略, 美国策略, DIRECT]

proxy-providers:
  Primary:
    <<: *base-provider
    url: "__SUB_URL_PLACEHOLDER__"
    override:
      additional-prefix: "[P] "
...
// ==================== Pro 模板 (包含自动选择分组) ====================
const PRO_TEMPLATE = `# 666OS Pro 自动选择增强版
...
x-direct: &direct [DIRECT, 自动选择, 故障转移, 全球手动, 香港策略, 台湾策略, 日本策略, 狮城策略, 韩国策略, 美国策略]
x-us-first: &us-first [美国策略, 自动选择, 故障转移, 全球手动, 香港策略, 台湾策略, 日本策略, 狮城策略, 韩国策略, DIRECT]
x-jp-first: &jp-first [日本策略, 自动选择, 故障转移, 全球手动, 香港策略, 台湾策略, 狮城策略, 韩国策略, 美国策略, DIRECT]
x-sg-first: &sg-first [狮城策略, 自动选择, 故障转移, 全球手动, 香港策略, 台湾策略, 日本策略, 韩国策略, 美国策略, DIRECT]
...
<body>
  <div class="container">
    ...
    <p class="subtitle">支持自动选择全量节点 · 包含自动测速与优化排序</p>
    ...
    <div class="card">
      ...
      <label>选择版本</label>
      <select id="version">
        <option value="lite">Lite（轻量推荐 - 包含自动选择）</option>
        <option value="pro">Pro（完整功能，含广告拦截与均衡分流）</option>
      </select>
      ...
      <div id="result">
        <div class="sub-link-box">
          <div class="sub-link-title">🔗 专属客户端一键订阅链接 (可直接填入 Clash / Mihomo 软件)：</div>
          <div class="sub-link-val" id="subClientUrl"></div>
        </div>

        <label>生成的 YAML 配置文本</label>
        <textarea id="yamlOutput" readonly></textarea>

        <div class="action-btns">
          <button class="copy-btn" onclick="copyYaml()">一键复制 YAML 文本</button>
    ...
    <div class="footer">
      666OS YAML SubConverter Service
    </div>
  </div>
  ...
  <script>
    ...
    async function generate() {
      ...
      const subClientUrl = document.getElementById('subClientUrl');
      ...
      try {
        ...
        // 设置一键客户端订阅链接 (Base64 编码原 URL 防特殊字符冲突)
        const encodedUrl = btoa(subUrl);
        const directSubUrl = window.location.origin + '/sub?url=' + encodeURIComponent(encodedUrl) + '&version=' + version;
        subClientUrl.textContent = directSubUrl;

        output.value = data.yaml;
        result.style.display = 'block';
        status.textContent = '生成成功！可复制下方直连订阅，或直接复制/下载配置！';
    ...
    function copyYaml() {
      ...
      document.getElementById('status').textContent = 'YAML 文本已复制到剪贴板！';
